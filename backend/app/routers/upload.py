...
# ── Response schema (defined here — no external schemas.py dependency) ────────
...
async def upload_crm_report(
    ...
    if not cases:
        raise HTTPException(
            status_code=400,
            detail="No case rows found. Check the file is a CRM closed-file report."
        )
...
async def upload_template_file(
    ...
    try:
        ...
        raise HTTPException(status_code=501,
                            detail="Template upload not yet available — use CRM upload.")
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Could not parse file: {str(e)}")
    finally:
        os.unlink(tmp_path)
    ...
    if errors:
        raise HTTPException(status_code=422, detail={"message": "Validation errors.", "errors": errors})

    run = Run(
        staff_name=staff_name, office=office,
        run_month=run_month, run_year=run_year,
        status="reviewed", input_file=file.filename,
        created_by=current_user.id,
        warnings=str(warnings) if warnings else None,
    )
    db.add(run)
    ...
    for cs in cases_parsed:
        ...
        case = Case(
            run_id=run.id,
            original_no=cs.original_no, student_name=cs.student_name,
            student_id=cs.student_id, contract_id=cs.contract_id,
            contract_date=cs.contract_date, client_type=cs.client_type,
            client_type_code=cs.client_type_code, country=cs.country,
            country_code=cs.country_code, agent=cs.agent,
            system_type=cs.system_type, app_status=cs.app_status,
            visa_date=cs.visa_date, institution=cs.institution,
            course_start=cs.course_start, course_status=cs.course_status,
            counsellor=cs.counsellor, case_officer=cs.case_officer,
            presales_agent=cs.presales_agent, incentive=cs.incentive,
            notes=cs.notes, service_fee_type=cs.service_fee_type,
            deferral=cs.deferral, package_type=cs.package_type,
            office_override=cs.office_override, handover=cs.handover,
            target_owner=cs.target_owner, case_transition=cs.case_transition,
    ...
    return UploadResponse(
        run_id=run.id, staff_name=run.staff_name,
        case_count=len(cases_parsed), flagged_count=flagged,
